...
const init = async () => {
    ...
    if ( "OpfsDb" in sqlite.oo1 && typeof sqlite.oo1.OpfsDb === "function" ) {
        ...
        db.exec( `CREATE TABLE IF NOT EXISTS pins ( embedding BLOB NOT NULL, isPinned INTEGER NOT NULL, screenshot TEXT NOT NULL, text TEXT NOT NULL, timestamp TEXT NOT NULL, url TEXT PRIMARY KEY NOT NULL );
        INSERT INTO pins ( embedding, isPinned, text, timestamp, screenshot, url ) VALUES ( $1, 1, '<p>Automatically generated title</p><p>and summary with <tag>tags</tag>.</p><p>And a screenshot below</p>', datetime( 'now' ), '${ exampleImage }', 'https://example.com' );`, { bind: [ new Float32Array().buffer ] } )
        return db
    } else
